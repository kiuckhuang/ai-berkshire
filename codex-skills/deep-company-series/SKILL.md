...
description: "AI Berkshire skill: 看懂XX公司（深度公司系列）：3-8 篇長文拆一家公司. Source: skills/deep-company-series.md."
---
...
- 每篇寫完先存 `local/reports/{公司名}/《看懂{公司名}》/0X-XX.md`
- **目錄衝突規則**：若 `local/reports/{公司名}/《看懂{公司名}》/` 已存在（舊版系列），不覆蓋、不混放——新系列寫入帶日期後綴的新文件夾 `local/reports/{公司名}/《看懂{公司名}》-{YYYYMMDD}/`，舊文件夾原樣保留
- 不立即推 GitHub——等用戶審閱
- 用戶提修訂意見後修改
- 修訂完才 git push
...
grep -r "<本機用戶名>\|/Users/\|<個人身份信息>" local/reports/ | head
```
